...
export const Poster = ({
  ...
  // const releaseDate = getYear(release_date);
  // const rating = twoDigitRating(vote_average * 10);
  ...
  return (
    <div className='relative flex rounded w-posterWidth lg:w-posterWidth-desktop aspect-poster lg:aspect-poster-desktop'>
      <Link
        href={`/${type}/${id}`}
        onClick={handleClick}
        className='w-posterWidth lg:w-posterWidth-desktop none-dragable'
      >
        <div
          style={bgStyle}
      ...
      >
        <div
          className={twJoin(
            "flex w-full gap-4",
            isFavorite ? "justify-end" : "justify-start",
          )}
        >
          {!isFavorite ? (
            <>
        ...
        <Link href={`/${type}/${id}`} onClick={handleClick}>
          <h2 className='py-2 font-bold text-base'>{title}</h2>
        </Link>
        <p className='flex flex-wrap gap-2 font-bold text-white text-sm uppercase'>
          <span className='px-1 sm:px-2 py-0.5 sm:py-px text-xs sm:text-sm rounded bg-green-600'>
            {twoDigitRating(rating * 10)} %
          </span>
          <span className='px-1 sm:px-2 py-0.5 sm:py-px text-xs sm:text-sm rounded bg-gray-800'>
            {getYear(releaseDate)}
          </span>
        </p>
