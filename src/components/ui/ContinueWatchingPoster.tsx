...
export const ContinueWatchingPoster = ({ tmdbId, type, title, posterPath, backdropPath, season, episode, size = 'backdrop' }: Props) => {
    ...
    >
      ...
      >
        <div
          ...
          className={twJoin('sm:hidden w-full h-full background-center rounded-lg overflow-hidden transitioning duration-300 group-hover:brightness-50 group-hover:scale-105')}
        />
        <div
          style={{
            backgroundImage: backdropPath || posterPath
          ...
          className={twJoin('hidden sm:flex w-full h-full background-center rounded-lg overflow-hidden transitioning duration-300 group-hover:brightness-50 group-hover:scale-105')}
        />
        <RxCross2 className='absolute top-3 right-3 text-[#D7D7D7] border border-[#D7D7D7] rounded-full w-8 h-8 p-2 flex justify-center items-center backdrop-brightness-50 backdrop-blur-md' onClick={(e) => {e.stopPropagation(); removeContinueWatchingVideo(tmdbId)}} />
