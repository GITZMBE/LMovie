...
import { fetchVideosByPerson } from "@/src/api";
import { ApiError, Genre, VideoType } from "@/src/models";
import genresJSON from "@/public/api/genres.json";

export const GET = async (req: Request, context: RouteContext<'/api/[type]/person/[id]'>) => {
  ...
  const videos = await fetchVideosByPerson(parseInt(id), (type as VideoType));
